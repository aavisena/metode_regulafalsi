...
with col1:
    ...
    max_iter = st.number_input("Iterasi maksimum:", value=100, step=1)
...
untuk menentukan akar persamaan non-linear.
""")
    st.markdown("</div>", unsafe_allow_html=True)
...
if hitung:
    ...
    try:
        ...
        if fa0 * fb0 > 0:
            st.error("❌ Interval tidak valid: f(a) dan f(b) harus berlainan tanda.")
        else:
            data = []
            a0, b0 = a, b
            akar = None
            ...
            with colR1:
                ...
                if akar is not None:
                    ...
                    st.error("Akar tidak ditemukan dalam batas iterasi.")
            ...
            with colR2:
                ...
                fig, ax = plt.subplots()
                ax.plot(df["Iterasi"], df["c"], marker="o", label="Nilai c")
                ax.set_xlabel("Iterasi")
                ax.set_ylabel("Perkiraan Akar")
                ...
                ax2.plot(x_plot, y_plot, label="f(x)")
                if akar is not None:
                    ax2.scatter(akar, f(akar), color="red", label="Akar")
                ax2.legend()
                ax2.grid(True)
        ...
        st.error(f"Terjadi kesalahan pada fungsi: {e}")

st.markdown("<div class='footer'>Dibuat dengan ❤️ menggunakan Streamlit • Regula Falsi</div>", unsafe_allow_html=True)
